...
class PirepUpdatedListener extends Listener
{
    ...
    public function handle(PirepUpdated $event)
    {
        ...
        try {
            $active_trip = TripReport::where(['completed' => false])->whereHas('users', function ($q) use ($user) {
                $q->where('user_id', $user);
            })->whereHas('flights', function (Builder $query) use ($flight) {
